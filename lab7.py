...
def testXOR():
    ...
    assert XOR(0,1) == 1
    assert XOR(1,0) == 1
    assert XOR(1,1) == 0 #Exclusive or so if both are on, it's 0
...
def gor3(x,y,z): #also Exclusive or
    '''Or of three inputs.'''
...
def FA(x,y,cin):
    ...
    return (gor(gand(y,cin), gand(x,XOR(y,cin))),XOR(x,(XOR(y,cin))))
...
def fourBitAdd(xxxx,yyyy):
    ...
    (c,zt) = FA(xxxx[2],yyyy[2],c)
    (c,zf) = FA(xxxx[1],yyyy[1],c)
    (c,ze) = FA(xxxx[0],yyyy[0],c)
